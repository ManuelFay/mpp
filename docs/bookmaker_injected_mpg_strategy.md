...
By default, bookmaker-injected runs report and log only the `transfer` variant
for elimination games. Draw bettor shares are shifted through the same `N-N` to
...
```bash
python3 bookmaker_injected_strategy.py input.csv --bettor-share-transfer auto
...
The default is `auto`: `transfer` only for elimination games, `no_transfer` for
non-elimination games. The prediction log stores the variant in
`bettor_share_transfer` and the fixture stage in `game_stage`. Use `both` only
