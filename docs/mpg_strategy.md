...
bookmaker-injected rank-1 pick. It uses the transfer-aware view: `transfer`
rows for elimination games and `no_transfer` rows for non-elimination games.
The run writes a histogram of simulated total points and marks the resolved
points in practice plus the simulated mean.
