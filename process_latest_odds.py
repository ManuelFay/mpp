...
def normalized_h2h_by_event(event_rows: list[dict[str, str]], market: str) -> tuple[dict[str, float], dict[str, float], int]:
    ...
    for outcomes in bookmaker_outcomes.values():
        ...
        if set(outcomes) != required_outcomes:
            continue
        if not odds_filters.valid_h2h_outcomes(outcomes):
            continue
...
def main() -> None:
    ...
    raw_rows = read_rows(args.in_file)
    rows = odds_filters.filter_snapshot_rows(raw_rows)
    output_rows = process_rows(rows, args.market)
    exact_score_rows = process_exact_scores(rows, args.market)
    multipliers, multiplier_rows = learn_score_shape_calibration(args.historical_odds_file)
    calibrated_exact_score_rows = calibrate_exact_score_rows(exact_score_rows, multipliers)
    write_rows(output_rows, args.out, OUT_FIELDS)
    write_rows(exact_score_rows, args.exact_score_out, SCORE_FIELDS)
    write_rows(calibrated_exact_score_rows, args.calibrated_exact_score_out, CALIBRATED_SCORE_FIELDS)
    write_rows(multiplier_rows, args.calibration_multipliers_out, CALIBRATION_MULTIPLIER_FIELDS)

    print(f"Input rows read: {len(raw_rows)}")
    print(f"Rows retained after odds filters: {len(rows)}")
    print(f"Games processed: {len(output_rows)}")
    print(f"Saved probabilities: {args.out}")
    print(f"Saved exact score probabilities: {args.exact_score_out}")
    print(f"Saved calibrated exact score probabilities: {args.calibrated_exact_score_out}")
    print(f"Saved score shape calibration multipliers: {args.calibration_multipliers_out}")
