...
5. Run the calculator with logging enabled unless the user explicitly says not to log:

```bash
.venv/bin/python bookmaker_injected_strategy.py <temporary-input.csv> --submission-id <stable-id> --logged-at-utc <timestamp>
```
...
8. Return the calculator's top five rows in the exact table format from the markdown. For elimination games, return only the `transfer` result.

## Output Format

Return one section per processed game:
...
- When running `simulate_bookmaker_injected.py`, expect only the transfer-aware simulation. It uses `transfer` rows for elimination games and `no_transfer` rows for non-elimination games.
